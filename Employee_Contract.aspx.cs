...
public partial class Employee_Contract : System.Web.UI.Page
{
        ...
        try
        {
            using (vt_EMSEntities db = new vt_EMSEntities())
            {
                ...
                else
                {
                  ...
                  //      emp.BranchID = Convert.ToInt32(ddlBranch.SelectedValue);
                        ...
                        {
                            emp.ImageName = "/images/Employees/ProfileImage/" + EmployeeId + "-" + uploadEmpImage.PostedFile.FileName;
                        }
                        else
                        {
                        ...
                        {
                            emp.NICImage = "/images/Employees/NicImage/" + EmployeeId + "-" + NICUpload.PostedFile.FileName;
                        }
                        else
                        {
                       ...
                       // string qualification = txtHidden_Academic.Value;
                        ...
                        //    obj_qualification.Marks = Convert.ToInt32(item.Marks);
            ...
            if (uploadEmpImage.HasFile)
            {
                    ...
                    Extenion.ToLower() == ".bmp")
                {
                    ...
                    uploadEmpImage.SaveAs(MapPath("~/images/Employees/ProfileImage/" + EmployeeIdd + "-" + ViewState["ImageName"].ToString()));
                ...
                if (NICUpload.HasFile)
                {
                    string Extenions = Path.GetExtension(uploadEmpImage.PostedFile.FileName);
                    if (Extenion.ToLower() == ".jpg" || Extenion.ToLower() == ".png" || Extenion.ToLower() == ".gif" || Extenion.ToLower() == ".jpeg" ||
                        Extenion.ToLower() == ".bmp")
                    {
                        ViewState["ImageName"] = NICUpload.PostedFile.FileName;
                        string ext = Extenion.Substring(1);
                        NICUpload.SaveAs(MapPath("~/images/Employees/NicImage/" + EmployeeIdd + "-" + ViewState["ImageName"].ToString()));
                        //empImageView.ImageUrl = "~/images/Employees/" + ViewState["ImageName"].ToString();
                        lblmsg.Text = "";
                    }
                    else
    ...
    {
        vt_EMSEntities db = new vt_EMSEntities();
        var EmployeeObj = db.vt_tbl_Employee.Where(x => x.Email.ToUpper() == Email.ToUpper()).SingleOrDefault();
        if (EmployeeObj != null)
        {
            return true;
        }
