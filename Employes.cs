...
public partial class Employes : System.Web.UI.Page
{
    ...
    private void FillDetailForm(int ID)
    {
        using (vt_EMSEntities db = new vt_EMSEntities())
        {
            ...
            using (vt_EMSEntities db = new vt_EMSEntities())
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        emp.AccountNo = vt_Common.CheckInt(txtaccount.Text);
                        ...
                        foreach (var item in listQualificationAcademic)
                        {
                            ...
                            obj_qualification.Marks = Convert.ToInt32(item.Marks).ToString();
  ...
  protected void Btnexcelimport_Click(object sender, EventArgs e)
     {
        ...
        List<VT_SP_Getdesignation_ByCompanyID_Result> DesignationList = new List<VT_SP_Getdesignation_ByCompanyID_Result>();
        DesignationList = GetdesignationByCompanyID(Convert.ToInt32(Session["CompanyId"]));
        Departlist = GetDepartment_ByCompanyID(Convert.ToInt32(Session["CompanyId"]));
        ReadExcel(DesignationList, Departlist);
        //   GETDepartID(Departdt, dt); 
	}
	public DataTable ReadExcel(List<VT_SP_Getdesignation_ByCompanyID_Result> DesignationList, List<VT_SP_GetDepartment_ByCompanyID_Result> Departlist)
	{
        ...
        try
        {
            if (FileExportExcel.HasFile)
            {
               ...
               // for (int i = 1; i < 2; i++)
                ...
                using (vt_EMSEntities dbs = new vt_EMSEntities())
                {
                    emp.CompanyID = Convert.ToInt32(Session["CompanyId"]);
                    emp.RoleID = 4;
                    dbs.vt_tbl_Employee.Add(emp);
                    dbs.SaveChanges();
                }
    ...
    public List<VT_SP_Getdesignation_ByCompanyID_Result> GetdesignationByCompanyID(int? CompanyID)
    {
        vt_EMSEntities db = new vt_EMSEntities();
        var objdesignation = db.VT_SP_Getdesignation_ByCompanyID(CompanyID);
