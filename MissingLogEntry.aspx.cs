...
public partial class MissingLogEntry : System.Web.UI.Page
{
    ...
    protected void grdMissingLogEntry_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        switch (e.CommandName)
        {
            case "DeleteCompany":
                try
                {
                    using (vt_EMSEntities db = new vt_EMSEntities())
                    {
                        ...
                        //db.vt_tbl_LoanEntry.Remove(pmr);
                        db.SaveChanges();
                        LoadData();
                        UpView.Update();
            ...
            case "EditCompany":
                ...
                vt_Common.ReloadJS(this.Page, "$('#missinglogentry').modal();binddata();");
                //UpDetail.Update();
                break;
            default:
                break;
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            using (vt_EMSEntities db = new vt_EMSEntities())
            {
                ...
                {
                    //db.vt_tbl_MissingLogEntry.Add(pmr);
                }
    ...
    void LoadData()
    {
        if (Session["EMS_Session"] != null)
        {
            using (vt_EMSEntities db = new vt_EMSEntities())
            {
                ...
                //var quer = (from ml in db.vt_tbl_PFMonthlyReturn
                //            //join emp in db.vt_tbl_Employee on pmr.EnrollId equals emp.EmployeeID                           
                //            where ml.CompanyID == vt_Common.CompanyId
                //            select new
                //            {
                //                ml.EnrollId,
                //                //emp.EmployeeName,
                ...
                //            }).ToList();
                //grdMissingLogEntry.DataSource = (quer);
                //grdMissingLogEntry.DataBind();
            }
        }
    ...
    void ClearForm()
    {
        ViewState["PageID"] = null;
        //vt_Common.Clear(pnlDetail.Controls);
        vt_Common.ReloadJS(this.Page, "$('#missinglogentry').modal('hide');$('.confirm').confirm();binddata();");
    }
